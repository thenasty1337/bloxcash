...
router.post('/', apiLimiter, isAuthed, async (req, res) => {
    ...
    const dataToEncrypt = {
        userId: crypto.randomUUID() // req.user.id
    };
    ...
    await sql.query('INSERT INTO cardDeposits (orderId, fiatAmount, userId) VALUES (?, ?, ?)', [orderId, amount, req.user.id]);
