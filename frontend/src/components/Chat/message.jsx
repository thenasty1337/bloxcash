        ...
        <>
            <div class={'chatmessage-container ' + props?.user?.role + (wasMentioned() ? ' mentioned' : '')}>
                ...
                <div class='user' onClick={() => setParams({ user: props?.user?.id })}>
                    <div class='avatar-container'>
                        {props?.user?.role != 'OWNER' && props?.user?.role != 'ADMIN' ? (
                            <Avatar id={props?.user?.id} xp={props?.user?.xp} height={30} avatar={props?.user?.avatar}/>
                        ) : (
            ...
            <style jsx>{`
              ...
              .level-badge .level {
                ...
                background: linear-gradient(135deg, #8b78dd, #7c6bbf) !important;
